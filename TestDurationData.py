...
def run_driving_tests(gmaps_client, from_places):
    __perform_duration_step_test(gmaps_client, from_places,
                                 to_places_leisure, 'driving', "leisure")
    __perform_duration_step_test(
        gmaps_client, from_places, to_places_work, 'driving', "work")


def run_subway_transit_tests(gmaps_client, from_places):
...
def run_transit_tests(gmaps_client, from_places):
    __perform_duration_step_test(gmaps_client, from_places,
                                 to_places_leisure, 'transit', "leisure")
    __perform_duration_step_test(
        gmaps_client, from_places, to_places_work, 'transit', "work")


# Private methods
def sleeper(test_func):
...
def __save_duration_dict_to_json(duration_data, mode, suffix):
    ...
    try:
        ...
        if e.errno != errno.EEXIST:
            print(
                "ERROR - can't create dir[{}] - something bad happens:{}".format(curr_dir, e))
        else:
            print(
    ...
    with open(file_name, "a", encoding='utf-8') as file:
        ...
        for tuple_value in sorted_places:
            formatted_tuple_value = (
                tuple_value[0], "{0:.2f}".format(tuple_value[1]))
