...
class GMapsGeoCodeParser(object):
    ...
    def get_status(self):
        return utility.get_status_code_from_response(self.geocode_result)

    def get_place_id(self):
        return GMapsGeoCodeParser._get_value_from_geocode(self.geocode_result, "place_id")
    ...
    def get_location(self):
        geometry_dict = GMapsGeoCodeParser._get_value_from_geocode(self.geocode_result, "geometry")
...
class GMapsDirectionDataParser(object):
    ...
    def get_status(self):
        return utility.get_status_code_from_response(self.direction_result)

    def get_duration(self):
        legs_list = utility.get_value_from_dict_list("legs", self.direction_result)
        if legs_list is None:
            return None

        duration_dict = utility.get_value_from_dict_list("duration", legs_list)
        if legs_list is None:
            return None
